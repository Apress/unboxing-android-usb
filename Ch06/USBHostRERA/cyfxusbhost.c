...
extern uint32_t prodCount, consCount, bufCount;
...
void CyFxApplnI2SInit() {
	CyU3PI2sConfig_t i2sConfig = {
			CyFalse, CyFalse, CyTrue, CY_U3P_I2S_PAD_MODE_NORMAL, CY_U3P_I2S_SAMPLE_RATE_44_1KHz, CY_U3P_I2S_WIDTH_16_BIT
	};
	CyU3PReturnStatus_t status;

	status = CyU3PI2sInit();
	if(status) {
		CyU3PDebugPrint(2, "I2S init failed %x\n", status);
		return;
	}

	status = CyU3PI2sSetConfig(&i2sConfig, NULL);
	if(status) {
		CyU3PDebugPrint(2, "I2S config failed %x\n", status);
		return;
	}
...
{
	CyU3PDebugPrint(2, "Host event callback: %d %d\n", evType, evData);
    ...
    {
    	//CyU3PDebugPrint (2, "DISCONNECT EVENT.\r\n");
        ...
        uint8_t *buffer_p)
{
    ...
    if (status != CY_U3P_SUCCESS)
    	CyU3PDebugPrint (2, "EP0 status is %d\r\n", status);
...
CyFxApplnStart ()
{
    ...
    vid = CY_U3P_MAKEWORD(glEp0Buffer[9], glEp0Buffer[8]) ;
    pid = CY_U3P_MAKEWORD(glEp0Buffer[11], glEp0Buffer[10]) ;

    pid_tmp  = pid & 0xFFF0;

    CyU3PDebugPrint (4, "VID %x,PID %x \r\n", vid,pid);
    ...
    if( (vid == 0x18D1) && (pid_tmp == 0x2d00) )
    {
    	CyU3PDebugPrint (4, "In AOA Start\r\n");
        ...
        uint32_t input)
{
	...
	CyU3PDmaBuffer_t left_buf, right_buf, in_buf;

    CyU3PEventCreate(&glDMAEvent);

    /* Initialize the debug logger. */
//    CyFxApplnDebugInit ();
    CyU3PDebugPrint (2, "Application initialized.\r\n");
...
//    	status = CyU3PEventGet(&glDMAEvent, PROD_EVENT, CYU3P_EVENT_AND, &flags, 1000);
//    	if(status == 0) {
//    		CyU3PDebugPrint(2, "prodCount -> %d; event -> 0x%x\n", prodCount, status);
//    	}

    	if(CyU3PDmaChannelGetBuffer(&glHostAoACh, &in_buf, 10) == 0)
    	{
    		uint32_t i, count = in_buf.count;
    		uint8_t * buf = in_buf.buffer;
    		lc = rc = 0;

			CyU3PDmaChannelGetBuffer(&glI2SLeftCh, &left_buf, 0);
			CyU3PDmaChannelGetBuffer(&glI2SRightCh, &right_buf, 0);

			for(i=0; i<count; i+=4) {
				left_buf.buffer[lc++] = buf[i];
				left_buf.buffer[lc++] = buf[1+i];
				right_buf.buffer[rc++] = buf[2+i];
				right_buf.buffer[rc++] = buf[3+i];
			}

			CyU3PDmaChannelDiscardBuffer(&glHostAoACh);
			CyU3PDmaChannelCommitBuffer(&glI2SLeftCh, count/2, 0);
			CyU3PDmaChannelCommitBuffer(&glI2SRightCh, count/2, 0);
    	}
